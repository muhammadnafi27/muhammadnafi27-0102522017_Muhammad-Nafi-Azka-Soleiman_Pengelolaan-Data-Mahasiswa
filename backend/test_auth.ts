...
async function runTests() {
  ...
  console.log('1. Login berhasil:', res.status === 200 && data.success === true ? 'PASS' : 'FAIL');
  ...
  let meData: any = await res.json();
  console.log('14. auth/me data user:', res.status === 200 && meData.success ? 'PASS' : 'FAIL');
